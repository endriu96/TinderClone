...
class TinderActivity : AppCompatActivity(), TinderCallback{
    ...
    //copy-paste from net as standard image storing code
    fun storeImage(){
        if(resultImageURL!=null && userId !=null){
            val filePath=FirebaseStorage.getInstance().reference.child("profileImage")
                .child(userId!!)
            ...
            uploadTask.addOnFailureListener{e->e.printStackTrace()}
                filePath.downloadUrl
                    .addOnCompleteListener { uri->profileFragment?.updateImageUri(uri.toString())
                    }
